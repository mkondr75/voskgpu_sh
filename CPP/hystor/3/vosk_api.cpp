...
// Используем void* — это универсальный указатель, который C# поймет как IntPtr
VOSK_API void* vosk_model_new(const char* model_path) {
    std::cout << "[NATIVE] Received model path: " << model_path << std::endl;
    // Пока возвращаем фиктивный адрес для теста Interop
    return (void*)0x12345678; 
}

VOSK_API void vosk_model_free(void* model) {
    std::cout << "[NATIVE] Model freed at address: " << model << std::endl;
}
